...
Usage:
    ...
    curl -fsSL https://raw.githubusercontent.com/${REPO}/main/install.sh | VERSION=4.2.8 bash
...
build_download_url() {
    ...
    case "$PLATFORM" in
        linux)
            case "$PKG_EXT" in
                deb)
                    # Antigravity.Tools_4.2.8_amd64.deb or _arm64.deb
                    ...
                    ;;
                rpm)
                    # Antigravity.Tools-4.2.8-1.x86_64.rpm or -1.aarch64.rpm
                    ...
                    ;;
                AppImage)
                    # Antigravity.Tools_4.2.8_amd64.AppImage or _aarch64.AppImage
                    local appimage_arch
